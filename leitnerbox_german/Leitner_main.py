...
class add_word_window(tk.Toplevel):
    ...
    def check_noun(self,event):
        if aw_pos.get() == 'Noun (n)':
            self.artikel_combobox['state'] = 'normal'
            self.artikel_combobox.set("")
        else:
            self.artikel_combobox.set("")
            self.artikel_combobox['state'] = 'disabled'
    ...
    def __init__(self):
        ...
        pos_list = ('Verb (V)', 'Noun (n)', 'Adjective (Adj)', 'Adverb (Adv)', 'Preposition (Prep)', 'Conjunction (Conj)')
        pos_combobox = ttk.Combobox(self, textvariable=aw_pos)
        pos_combobox.bind("<<ComboboxSelected>>",self.check_noun)
        pos_combobox['values'] = pos_list
        pos_combobox.current(0)
        pos_combobox.grid(row=1,column=1)

        # Artikel Part (If Noun)
        ttk.Label(self , text='Artikel of the Word (If Noun):').grid(row=2,column=0)
        artikel_list = ('Der', 'Die', 'Das')
        self.artikel_combobox = ttk.Combobox(self, textvariable=aw_artikel)
        self.artikel_combobox['values'] = artikel_list
        self.artikel_combobox['state'] = "disabled"
        self.artikel_combobox.grid(row=2,column=1)


        # Name part
        ttk.Label(self , text='Name of the Word:').grid(row=3,column=0)
        name_entry = ttk.Entry(self, textvariable=aw_name).grid(row=3,column=1)

        # Definition part
        ttk.Label(self , text='Definition of the Word:').grid(row=4,column=0)
        def_entry = ttk.Entry(self, textvariable=aw_def).grid(row=4,column=1,sticky='nwes')

        # Example part
        global aw_ex
        ttk.Label(self , text='Example in a sentence (Optional)').grid(row=5,column=0)
        aw_ex = tk.Text(self, width=32, height=8)
        aw_ex.grid(row=5,column=1)

        # OK Button
        ttk.Button(self, text='Add this word to the dictionary.' , command=self.addWord).grid(row=6,column=0,columnspan=2)
    ...
    def addWord(self):
        ...
        new_ex = aw_ex.get("1.0",'end-1c')
        if new_pos == "Noun (n)":
            new_artikel = aw_artikel.get().capitalize()
        ...
        if new_name=="":
            ...
            halt=True
        elif new_pos == "Noun (n)" and new_artikel=="":
            messagebox.showerror('Error!','Please fill the artikel of the word.',parent=self)
            halt=True
        else:
            for x in range(8):        
...
class add_edit_window(tk.Toplevel):
    def __init__(self):
        ...
        # Artikel part
        ttk.Label(self , text='Artikel of the Word (If Noun):').grid(row=5,column=0)
        artikel_list = ('Der', 'Die', 'Das')
        self.artikel_combobox = ttk.Combobox(self, textvariable=edw_artikel)
        self.artikel_combobox['values'] = artikel_list
        self.artikel_combobox.grid(row=5,column=1)


        # Definition part
        ttk.Label(self , text='Definition of the Word:').grid(row=6,column=0)
        self.def_entry = ttk.Entry(self, textvariable=edw_def)
        self.def_entry.grid(row=6,column=1,sticky='nwes')
        
        # Example part
        global edw_ex
        ttk.Label(self , text='Example in a sentence (Optional)').grid(row=7,column=0)
        edw_ex = tk.Text(self, width=32, height=8)
        edw_ex.grid(row=7,column=1)

        # Delete button
        self.delete_button = ttk.Button(self, text='Delete' , command=self.deleteWord)
        self.delete_button.grid(row=8,column=0,pady=6,padx=4)

        # OK Button
        self.edit_ok_button = ttk.Button(self, text='Complete Editing' , command=self.editWord)
        self.edit_ok_button.grid(row=8,column=1,pady=6,padx=4)
    ...
    def editWord(self):
        ...
        if my_dict['Pos'] == "Noun (n)":
            my_dict["Artikel"] = edw_artikel.get().capitalize()
        my_dict['Definition'] = edw_def.get().capitalize()
        my_dict['Example'] = edw_ex.get("1.0",'end-1c')
        ...
        if my_dict['Name']=="":
            ...
            messagebox.showerror('Error!','Please fill the definition of the word.',parent=self)
        elif my_dict['Pos'] == "Noun (n)" and my_dict["Artikel"]=="":
            messagebox.showerror('Error!','Please fill the artikel of the word.',parent=self)
        else:
    ...
    def enable_edit_positions(self):
        ...
        if  edw_pos.get() == "Noun (n)":
            self.artikel_combobox['state'] = 'normal'
        else:
            self.artikel_combobox['state'] = 'disable'
...
def screen_play():
    ...
    if selected_pos == "Noun (n)":
        selected_artikel = selected_words[current_index]['Artikel'] 
    else:
        selected_artikel = ""
    selected_ex  = selected_words[current_index]["Example"]
    if selected_ex == "":
        txt_file = f'{selected_pos} -- {selected_artikel} {selected_name} \n'
    else:
        txt_file = f'{selected_pos} -- {selected_artikel} {selected_name} \nEXAMPLE: {selected_ex}'
